...
<div class="container-fluid px-3">
    ...
    <div class="row g-3 mb-4">
        <div class="col-6 col-md-3">
            <div class="stat-card primary">
                <div class="label">Total Stok Bersih</div>
                <div class="value">{{ number_format($totalStok ?? 0, 0, ',', '.') }} <span class="unit">Kg</span></div>
                <small class="text-muted">{{ $jenisProdukCount ?? 0 }} jenis produk</small>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-card success">
                <div class="label">Produk Masuk (Bulan Ini)</div>
                <div class="value">{{ number_format($stokMasukBulanIni ?? 0, 0, ',', '.') }} <span class="unit">Kg</span></div>
                <small class="text-muted">Dari hasil produksi</small>
            </div>
        </div>
        <div class="col-6 col-md-3">
            <div class="stat-card warning">
                <div class="label">Produk Keluar (Bulan Ini)</div>
                <div class="value">{{ number_format($stokKeluarBulanIni ?? 0, 0, ',', '.') }} <span class="unit">Kg</span></div>
                <small class="text-muted">Dari penjualan</small>
            </div>
        </div>
        <div class="col-6 col-md-3">
    ...
    <div class="alert alert-warning border-0 shadow-sm rounded-3 mb-3">
        <div class="d-flex align-items-center">
            ...
            <div>
                ...
                @if(($stokHabis ?? 0) > 0)
                    Ada {{ $stokHabis }} jenis produk yang stoknya habis.
                @endif
                @if(($stokMenipis ?? 0) > 0)
                    Ada {{ $stokMenipis }} jenis produk yang stoknya menipis (&lt; 100 Kg).
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="filter-bar">
        <form method="GET" action="{{ route('produksi.stok.index') }}" class="row g-2 align-items-end">
            <div class="col-md-4">
            ...
            <div class="col-md-5 text-end">
                ...
                <a href="{{ route('produksi.stok.index') }}" class="btn btn-outline-secondary btn-sm rounded-pill px-4">
                ...
                <table class="table table-hover mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-end">Stok Bersih (Kg)</th>
                            <th>Level</th>
                            <th>Status</th>
                    ...
                    <tbody>
                        @forelse($stok as $index => $item)
                            @php
                                ...
                                // Debug: pastikan nilai ada
                                $stokMasuk  = isset($item->stok_masuk) ? (float) $item->stok_masuk : 0;
                                $stokKeluar = isset($item->stok_keluar) ? (float) $item->stok_keluar : 0;
                                $totalBerat = isset($item->total_berat) ? (float) $item->total_berat : max(0, $stokMasuk - $stokKeluar);
                                
                                // Hitung persentase
                                $persentase = $totalBerat > 0
                                    ? min(100, ($totalBerat / $maxIdeal) * 100)
                                    : 0;
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <span class="{{ $stokMasuk > 0 ? 'text-success fw-semibold' : 'text-muted' }}">
                                        {{ number_format($stokMasuk, 2, ',', '.') }}
                                    </span>
                                </td>
                                <td class="text-end">
                                    <span class="{{ $stokKeluar > 0 ? 'text-danger fw-semibold' : 'text-muted' }}">
    ...
    <a href="{{ route('produksi.stok.riwayat', $item->jenis_produk_id) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-5">
                                    ...
                                    <small class="text-muted">Stok akan bertambah setelah ada input hasil produksi</small>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
