...
<div class="container-fluid px-2 px-md-3" style="max-width:800px;margin:0 auto;">
    ...
    <form action="{{ route('penjualan.update', $penjualan->id) }}" method="POST" id="formEdit">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                ...
                <div class="row g-2">
                    <div class="col-6">
                        ...
                        <input type="date" name="tanggal" id="tanggal" class="form-control" value="{{ old('tanggal', $penjualan->tanggal->format('Y-m-d')) }}" required>
                    </div>
                    <div class="col-6">
        ...
        <div class="d-flex gap-2 mt-2">
            ...
            <button type="button" class="btn-submit flex-fill" id="btnSimpan" onclick="simpanUpdate()"><i class="fas fa-save me-1"></i>Simpan Perubahan</button>
        </div>
    </form>
</div>
...
function loadExisting() {
    existingDetail.forEach(d => {
        tambahProduk(d.jenis_produk_id, d.jumlah_sak, d.harga_per_kg, d.berat_nett_kg, d.berat_kirim_kg);
    });
    if (existingDetail.length === 0) tambahProduk();
}

// ========== TAMBAH PRODUK ==========
function tambahProduk(selectedId = '', existingSak = 1, existingHarga = '', existingNett = '', existingKirim = '') {
    let opt = '<option value="">-- Pilih Produk --</option>';
    jenisProdukList.forEach(p => opt += `<option value="${p.id}" data-stok="${p.stok_tersedia}" ${p.id == selectedId ? 'selected' : ''}>${p.nama} (Stok: ${formatNum(p.stok_tersedia)} Kg)</option>`);
    ...
    <div class="produk-group" id="produk${newIdx}">
        ...
        <label class="form-label">Sak/Karung (Berat Kirim)</label>
        ...
        <div style="font-size:10px;color:#888;margin-top:2px;" id="kirimSummary${newIdx}">Total Berat Kirim: <strong>0 Kg</strong></div>
        <div class="stok-warning" id="stokWarning${newIdx}"></div>
        
        <div style="margin:12px 0 6px;">
            <label class="form-label">Berat Nett (Timbangan Pembeli)</label>
        </div>
        <div class="row g-2">
    ...
    if (existingKirim > 0 && existingSak > 0) {
        const beratPerSak = existingKirim / existingSak;
        for (let i = 0; i < existingSak; i++) {
            tambahSak(newIdx, i === 0 ? existingKirim : 0);
        }
        // Update nilai sak
        const sakInputs = sakList.querySelectorAll('input');
        if (sakInputs.length > 0) {
            sakInputs[0].value = existingKirim;
        }
    } else {
        tambahSak(newIdx);
    }
...
function hapusProduk(idx) {
    ...
    if (all.length <= 1) return Swal.fire({ icon: 'warning', title: 'Minimal 1 produk!', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 });
...
function renumberProduk() {
    ...
    all.forEach((g, i) => {
        ...
        if (produkSelect) { produkSelect.setAttribute('name', `items[${newId}][jenis_produk_id]`); produkSelect.setAttribute('onchange', `updateInfo(${newId})`); }
        ...
        g.querySelectorAll('.btn-add-sm').forEach(btn => { if (btn.textContent.includes('Sak')) btn.setAttribute('onclick', `tambahSak(${newId})`); });
        ['kirimSummary', 'stokWarning', 'maxNett', 'nettWarning'].forEach(base => { const el = g.querySelector(`[id^="${base}"]`); if (el) el.id = base + newId; });
        const nettInput = g.querySelector('.nett-input');
        if (nettInput) { nettInput.setAttribute('name', `items[${newId}][berat_nett_kg]`); nettInput.setAttribute('oninput', `cekBatasanNett(this);hitungTotal();`); }
        ['calcSak', 'calcKirim', 'calcNett', 'calcPotonganKg', 'calcPotonganPersen', 'calcHarga', 'calcSubtotal', 'calcCard', 'rowPotongan', 'rowPotonganPersen'].forEach(idName => { const el = g.querySelector(`[id^="${idName}"]`); if (el && el.id.includes(parseInt(g.id.replace('produk','')))) el.id = idName + newId; });
    });
...
function hitungTotal() {
    ...
    document.querySelectorAll('.produk-group').forEach((g) => {
        ...
        document.getElementById('kirimSummary' + idx).innerHTML = `Total Berat Kirim: <strong>${formatNum(gKirim)} Kg</strong> dari <strong>${gSak} Sak</strong>`;
        const stokWarn = document.getElementById('stokWarning' + idx);
        if (stokWarn) stokWarn.style.display = (gKirim > stok && stok > 0 && sel?.value) ? 'block' : 'none';
        
        const updates = { calcSak: gSak + ' Sak', calcKirim: formatNum(gKirim) + ' Kg', calcNett: formatNum(beratNett) + ' Kg', calcHarga: 'Rp ' + formatRupiah(harga), calcSubtotal: 'Rp ' + formatRupiah(subtotal), calcPotonganKg: formatNum(beratPotongan) + ' Kg', calcPotonganPersen: formatNum(potonganPersen) + '%' };
